...
# Evaluation CAUSAL FOREST Fashion A -----------------------------------
...
cf_f_a_preds = read.csv2("/Users/asmir/hu_apa/APA_predictions/cf_preds/cf_f_a_preds.csv")
head(cf_f_a_preds)
cf_f_a_preds = cf_f_a_preds[,2]
cf_f_a_preds

#table(cf_hillstrom_preds != 0)
# We rank the uplift scores from high to low and add this infromation to a dataframe 
# We rank the uplift scores from high to low and add this infromation to a dataframe 
mm_cf = cbind(uplift = cf_f_a_preds,
              target = f_a.test$checkoutAmount,
              treatment = f_a.test$treatment,
              uplift_rank = length(cf_f_a_preds) +1 - rank(cf_f_a_preds, ties.method = "random"))
...
ggplot(temp.df_cf.uplift, aes(x=Deciles)) +
  ...
  scale_y_continuous(labels=, limits=c(-2,2), name="Uplift (Treatment - Control) in Average Spend") +
  scale_x_discrete(name ="Deciles", limits=rep(1:10)) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank()) +
...
# Evaluation BART Fashion A

bart_f_a_preds = read.csv("/Users/asmir/hu_apa/APA_predictions/bart_preds/bart_f_a_preds.csv")
head(bart_f_a_preds)
bart_f_a_preds = bart_f_a_preds[,2]
bart_f_a_preds

#table(bart_hillstrom_preds != 0)
# We rank the uplift scores from high to low and add this infromation to a dataframe 
# We rank the uplift scores from high to low and add this infromation to a dataframe 
mm_bart = cbind(uplift = bart_f_a_preds,
                target = f_a.test$checkoutAmount,
                treatment = f_a.test$treatment,
                uplift_rank = length(bart_f_a_preds) +1 - rank(bart_f_a_preds, ties.method = "random"))
...
ggplot(temp.df_bart.uplift, aes(x=Deciles)) +
  geom_bar(stat="identity", aes(y =Uplift, fill="red")) + 
  scale_y_continuous(labels=, limits=c(-1,2), name="Uplift (Treatment - Control) in Average Spend") +
  scale_x_discrete(name ="Deciles", limits=rep(1:10)) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank()) +
