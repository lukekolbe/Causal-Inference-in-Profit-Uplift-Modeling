...
# CAUSAL FOREST FASHION B EVALUATION  ------------------------------
cf_f_b_preds = read.csv2("/Users/asmir/hu_apa/APA_predictions/cf_preds/cf_f_b_preds.csv")
head(cf_f_b_preds)
cf_f_b_preds = cf_f_b_preds[,2]
cf_f_b_preds

#table(cf_hillstrom_preds != 0)
# We rank the uplift scores from high to low and add this infromation to a dataframe 
# We rank the uplift scores from high to low and add this infromation to a dataframe 
mm_cf = cbind(uplift = cf_f_b_preds,
              target = f_b.test$checkoutAmount,
              treatment = f_b.test$treatment,
              uplift_rank = length(cf_f_b_preds) +1 - rank(cf_f_b_preds, ties.method = "random"))
...
ggplot(temp.df_cf, aes(x=Decile)) +
  ...
  scale_y_continuous(labels=, limits=c(0,12), name="Average Revenue (Euro)") +
...
# BART FASHION B EVALUATION -------------------------------------


bart_f_b_preds = read.csv("/Users/asmir/hu_apa/APA_predictions/bart_preds/bart_f_b_preds.csv")
head(bart_f_b_preds)
bart_f_b_preds = bart_f_b_preds[,2]
bart_f_b_preds

#table(bart_hillstrom_preds != 0)
# We rank the uplift scores from high to low and add this infromation to a dataframe 
# We rank the uplift scores from high to low and add this infromation to a dataframe 
mm_bart = cbind(uplift = bart_f_b_preds,
                target = f_b.test$checkoutAmount,
                treatment = f_b.test$treatment,
                uplift_rank = length(bart_f_b_preds) +1 - rank(bart_f_b_preds, ties.method = "random"))
