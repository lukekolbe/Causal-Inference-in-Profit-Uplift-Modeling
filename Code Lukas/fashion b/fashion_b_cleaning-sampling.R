...
#f_b <- f_b[,-which(names(f_b) %in% c("controlGroup","campaignMov","campaignValue","label","NormalizedCartSum"))] 
...
#                                    , size = round(0.15*sample_size_f_b[i]), replace=FALSE) 
# } 
# 
# 
...
fa_niv <- niv(f_niv_fa, f_b, subset=NULL, na.action = na.pass, B = 10, direction = 1, 
...
f_b.train_SMOTE <- SMOTE(f_smote_f_b,f_b.train_small,perc.over = 400,perc.under = 200)
