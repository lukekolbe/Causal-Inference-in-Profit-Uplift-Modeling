  ...
  for(i in 1:4){
    train_indices_b_t[[i]] <- sample(which(b_t$converted == combinations$Conversion[i] &
                                     ...
                                     , size = round(0.25*sample_size_b_t[i]), replace=FALSE) 
  } 
  ...
  set.seed(1)
  system.time(rfe_b_t.results <- rfe(trainData_b_t2[,-c(1,2)], trainData_b_t2[,1], sizes=subsets, rfeControl=control, maximize=TRUE))
