...
model.zoo <- c("2Model_Ridge","causalTree", "causalForest", "causalBart", "causalBoosting", "ensemble")

qini.matrix_28 <- setNames(data.frame(matrix(ncol = 6, nrow = 4)), model.zoo)
rownames(qini.matrix_28) <- data.zoo

max.profit.model <- setNames(data.frame(matrix(ncol = 6, nrow = 4)), model.zoo)
rownames(max.profit.model) <- data.zoo

treatment.decision <- setNames(data.frame(matrix(ncol = 6, nrow = 4)), model.zoo)
rownames(treatment.decision) <- data.zoo

# loading data F_A 25 features ------------------------------------------------------------

f_a.test <- read.csv("working data/eval/f_a_test_eval.csv")[,-1]

f_a_pred.matrix <- setNames(data.frame(matrix(ncol = 6, nrow = nrow(f_a.test))), model.zoo)
...
f_a_pred.matrix$ensemble <- rowMeans(f_a_pred.matrix[,1:5])
...
f_a_eval_FULL <- list(f_a_eval.2M, f_a_eval.CF, f_a_eval.CT, f_a_eval.BC, f_a_eval.CB, f_a_eval.ensemble, test.data=f_a.test)
...
f_b_pred.matrix <- setNames(data.frame(matrix(ncol = 6, nrow = nrow(f_b.test))), model.zoo)
...
f_b_pred.matrix$ensemble <- rowMeans(f_b_pred.matrix[,1:5])
...
f_b_eval_FULL <- list(f_b_eval.2M,f_b_eval.CT, f_b_eval.CF, f_b_eval.BC, f_b_eval.CB, f_b_eval.ensemble, test.data=f_b.test)
...
b_t_pred.matrix <- setNames(data.frame(matrix(ncol = 6, nrow = nrow(b_t.test))), model.zoo)
...
b_t_pred.matrix$ensemble <- rowMeans(b_t_pred.matrix[,c(1,3:5)]) # ensemble without causaltree
...
b_t_eval_FULL <- list(b_t_eval.2M, b_t_eval.CF, b_t_eval.BC, b_t_eval.CB, b_t_eval.ensemble, test.data=b_t.test)
...
h_s_pred.matrix<- setNames(data.frame(matrix(ncol = 6, nrow = nrow(h_s.test))), model.zoo)
#can only add four sets since B_T needed different (smaller) training sets for CausalTree, as an additional estimation set was needed...
...
h_s_pred.matrix$ensemble <- rowMeans(h_s_pred.matrix[,c(1,3,4,5)]) # ensemble without causaltree
